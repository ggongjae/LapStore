...
const PriceComparison = () => {
    ...
    if (products) {
        filteredAndSortedProducts = products
            .filter(product => convertToNumberPrice(product.가격) >= minPrice && convertToNumberPrice(product.가격) <= maxPrice)
            .sort((a, b) => convertToNumberPrice(a.가격) - convertToNumberPrice(b.가격));
    }
    ...
    return (
        <div className="product-comparison-container">
            ...
            {filteredAndSortedProducts.slice(0, visible).map((product, index) => (
            ...
            <button onClick={showMore}>{showAll ? '원상복귀' : '더보기'}</button>
        </div>
    );
};
