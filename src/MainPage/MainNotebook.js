...
const MainNotebook = () => {
    ...
    useEffect(() => {
        ...
        const videoIds = [
            'VM9eGyOMkyc', 'qcnXXhEiCVY', 'U6SUnX1kt30',
            'q5nyKSJtCc8', 'rWL-re30OKE', 'bCwTqO5NfkA',
            'nGPYvLK2R-E', '1rtD8sXcXpI', 'tZV_MeKoPis',
            'NGk3CW8jPvc', '1-7GmmSAljg', '0aRVMvlIyJg'
        ]; // 실제 영상 ID로 대체하세요
        const maxResults = 6;
        ...
        Promise.all(
            shuffleArray(videoIds).slice(0, maxResults).map((videoId) => {
            ...
            .then((responses) => {
                const videoData = responses.map((res) => ({
                    ...res.data.items[0],
                    formattedPublishedAt: formatPublishedAt(res.data.items[0].snippet.publishedAt),
                }));
                setVideos(videoData);
            })
            .catch((error) => {
    ...
    const formatPublishedAt = (publishedAt) => {
       ...
       if (timeDiff < 1000 * 60 * 60 * 24) { // 24시간 이내
            const hours = Math.floor(timeDiff / (1000 * 60 * 60));
            return `${hours}시간 전`;
        } else if (timeDiff < 1000 * 60 * 60 * 24 * 30.44 ) { // 1개월 이내
            const days = Math.floor(timeDiff / (1000 * 60 * 60 * 24));
            return `${days}일 전`;
        } else if (timeDiff < 1000 * 60 * 60 * 24 * 365 * 2) { // 2년 이내
        ...
        <div>
            <div className="notebook_information">
                <div className="notebook_information_content">
                    <p className='notebook_information_content_title'>노트북 정보 알아보기</p>
                    <a href='' className='notebook_information_content_more'>더많은 영상 보기 <FontAwesomeIcon icon={faChevronRight} /></a>
